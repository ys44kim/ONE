                ...
                const Shape &output_shape, T *output_data)
{
  ...
  const RoPEMode mode = params.mode;

  if(mode == RoPEMode::GPT_NEOX)
  {
      ...
      for (int32_t i1 = 0; i1 < i1_n; ++i1)
      {
        for (int32_t i2 = 0; i2 < i2_n; ++i2)
        {
          for (int32_t i3 = 0; i3 < i3_n/2; ++i3)
          {
            ...
            output_data[offset + i3_n/2] = x0 * sin_table_data[i3 + i3_n/2] + x1 * cos_table_data[i3 + i3_n/2];
          }
        }
      }
    }
  } else if(mode == RoPEMode::GPT_J)
  {
