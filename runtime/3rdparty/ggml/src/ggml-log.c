...
#define MAX_CORE_N      32
#define LOG_MAX_SIZE    5012
...
static int32_t min_core_id = 1024; // minimum core id for logging
static int32_t max_core_id = 0; // maximum core id for logging
...
void ggml_log_write(int32_t line, int64_t arg0, int64_t arg1, int64_t arg2, int64_t arg3)
{
  ...
  if(__builtin_expect(log_core_id < min_core_id, 0)) {
    min_core_id = log_core_id;
  }

  if(__builtin_expect(log_core_id > max_core_id, 0)) {
    max_core_id = log_core_id;
  }
...
{
  for(int core_id = min_core_id; core_id <= max_core_id; core_id++)
  {
    ...
    {
      int is_found = 0;
      for(int id = min_core_id; id <= max_core_id; id++) {
        if(id == core_id) {
          continue;
        }
        if(log_buffer[id][log_tail[id]].ts.tv_sec == 0 && log_buffer[id][log_tail[id]].ts.tv_nsec == 0) {
          continue; // empty log
        }
        if(compare_time(&log_buffer[core_id][log_tail[core_id]].ts, &log_buffer[id][log_tail[id]].ts) == 1) {
          is_found = 1;
          break; // find next log
        }
      }
