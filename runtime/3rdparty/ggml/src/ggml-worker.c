...
static void *worker(void *arg)
{
  ...
  const uint64_t cpuset = 0x1 << id;

  if ( pthread_setaffinity_np(pthread_self(), sizeof(cpuset), (cpu_set_t *)&cpuset) < 0 )
  {
...
void ggml_worker_submit(void (*func)(void *), void *arg)
{
  ...
  TaskQueue *queue = gQueue[current_worker_id];

  while ((queue->head + 1) % queue->capacity == queue->tail && !queue->stop)
  {
    pthread_cond_wait(&queue->cond, &queue->mutex);
  }
